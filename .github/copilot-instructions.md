...
- Activate the venv first (not automatic): `source .venv/bin/activate`. Your
  prompt should show `(.venv)` once it's active.
- `pip install <pkg>` to add dependencies. There's no `requirements.txt` yet —
  create one once you add your first dependency: `pip freeze > requirements.txt`.
  Update it the same way after each new install.
